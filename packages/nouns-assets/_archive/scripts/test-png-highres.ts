...
async function testResolution(resolution: number, paletteSize: number) {
  ...
  console.log(`  SVG通常: ${(normalTotal/1024).toFixed(1)}KB → ${(gasNormal/1e6).toFixed(1)}M gas`);
  console.log(`  SVG最大: ${(maxTotal/1024).toFixed(1)}KB → ${(gasMax/1e6).toFixed(1)}M gas ${v}`);
