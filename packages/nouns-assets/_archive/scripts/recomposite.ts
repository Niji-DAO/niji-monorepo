...
async function recomposite(dirLabel: string, resolution: number) {
  ...
  if (!fs.existsSync(dir)) { console.log(`${dir} not found`); return; }
  const allFiles = fs.readdirSync(dir).filter(f => f.endsWith('.png') && !f.includes('composite') && !f.includes('sample') && !f.includes('upscaled'));
  ...
  for (const trait of COMPOSITE_ORDER) {
    ...
    let bestFile = traitFiles[0], bestOp = 0;
    for (const f of traitFiles.slice(0, 10)) {
      const { data } = await sharp(path.join(dir, f)).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  ...
  await sharp({ create: { width: resolution, height: resolution, channels: 4, background: { r: 213, g: 215, b: 225, alpha: 1 } } })
    .composite(layers).png().toFile(compPath);
