...
async function generateVisual(config: TestConfig) {
  ...
  for (const trait of COMPOSITE_ORDER) {
    ...
    for (const p of pngs.slice(0, 15)) {
      ...
      if (op > bestOp) { bestOp = op; bestPng = p; }
    }
    ...
    svgParts.push(`<image width="${resolution}" height="${resolution}" href="data:image/png;base64,${b64}"/>`);
  ...
  console.log(`  SVG最大: ${(svgMaxSize/1024).toFixed(1)}KB → ${(gasMax/1e6).toFixed(1)}M gas ${v}`);
