...
async function runTest(config: TestConfig) {
  ...
  if (resolution > 255) { console.log('  スキップ (>255)'); return null; }
  ...
  for (const sf of sourceFiles) {
    let pipeline = sharp(sf.inputPath)
  ...
  return {
    label, resolution, paletteSize, blurRadius,
...
async function main() {
  ...
  console.log('========================================\n');
  console.log('テスト              | 解像度 | 色 | blur | SVG通常 | SVG最大 | ガス通常 | ガス最大 | 判定');
  console.log('--------------------|--------|-----|------|--------|--------|---------|---------|-----');
