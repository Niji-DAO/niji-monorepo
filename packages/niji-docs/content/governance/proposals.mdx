...
- A proposal with `0` against votes has a quorum of <code className="nextra-code">{readNijiGovernorMinQuorumVotes(wagmiConfig, {})}</code> votes
- The quorum increases by <code className="nextra-code">{quorumIncreasePerAgainstVote()}</code> vote per against vote
- The quorum reaches the max of <code className="nextra-code">{readNijiGovernorMaxQuorumVotes(wagmiConfig, {})}</code> votes with <code className="nextra-code">{maxQuorumAgainstVotes()}</code> against votes
