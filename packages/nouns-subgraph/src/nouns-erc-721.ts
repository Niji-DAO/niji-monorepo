...
import { DelegationEvent, Niji, Seed, TransferEvent } from './types/schema';
...
export function handleNounCreated(event: NounCreated): void {
  ...
  if (noun == null) {
    log.error('[handleNounCreated] Niji #{} not found. Hash: {}', [
      nounId,
...
export function handleDelegateChanged(event: DelegateChanged): void {
  ...
  // Log a transfer event for each Niji
  for (let i = 0; i < accountNouns.length; i++) {
...
export function handleTransfer(event: Transfer): void {
  ...
  let noun = Niji.load(transferredNounId);
  if (noun == null) {
    noun = new Niji(transferredNounId);
  }
