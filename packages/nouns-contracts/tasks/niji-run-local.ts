...
task(
  ...
  const contracts = await run('niji-deploy-local');

  await run('niji-populate-descriptor', {
  ...
  console.log(`Niji DAO Executor address: ${contracts.NounsDAOExecutor.instance.address}`);
  console.log(`Niji DAO Proxy address: ${contracts.NounsDAOProxyV2.instance.address}`);
