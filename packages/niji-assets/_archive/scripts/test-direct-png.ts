...
async function testDirectPng(resolution: number, paletteSize: number, useQuantize: boolean) {
  ...
  for (const trait of COMPOSITE_ORDER) {
    ...
    for (const f of files) {
      ...
      const buf = await sharp(data, { raw: { width: resolution, height: resolution, channels: 4 } })
        .png()
        .toBuffer();
  ...
  console.log(
    `  中央値PNG: ${(median / 1024).toFixed(1)}KB → base64: ${(normalB64 / 1024).toFixed(1)}KB → ${(gasNormal / 1e6).toFixed(1)}M gas`,
  );
  console.log(
    `  95%ile PNG: ${(p95 / 1024).toFixed(1)}KB → base64: ${(p95B64 / 1024).toFixed(1)}KB → ${(gasP95 / 1e6).toFixed(1)}M gas`,
  );
  console.log(
    `  最大PNG: ${(max / 1024).toFixed(1)}KB → base64: ${(maxB64 / 1024).toFixed(1)}KB → ${(gasMax / 1e6).toFixed(1)}M gas`,
  );
...
async function main() {
  ...
  console.log('========================================\n');
  console.log(
    'テスト                  | 中央値   | 95%ile   | 最大     | ガス中央 | ガス最大 | 判定',
  );
  console.log('------------------------|---------|---------|---------|---------|---------|-----');
