...
task(
  'test-niji-png',
  'Deploy NijiArt + NijiDescriptor and measure real gas for tokenURI',
  ...
  const traitIndices = TRAIT_DIRS.map(t =>
    samplePngs.has(t.id) ? 0 : ethers.constants.MaxUint256,
  );
  ...
  try {
    ...
    console.log(
      `   SVG result length: ${svgResult.length} chars = ${(svgResult.length / 1024).toFixed(1)}KB`,
    );
