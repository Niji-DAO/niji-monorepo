...
describe('shortENS — boundary', () => {
  ...
  it('round-11 30 sequential formatShortAddress truthiness', () => {
    for (let i = 0; i < 30; i++) {
      expect(formatShortAddress).toBeTruthy();
    }
  });

  it('round-11 30 sequential shortENS truthiness', () => {
    for (let i = 0; i < 30; i++) {
      expect(shortENS).toBeTruthy();
    }
  });
  ...
  it('round-11 50 sequential format invocations', () => {
    for (let i = 0; i < 50; i++) {
      const addr = ('0x' + i.toString(16).padStart(40, '0')) as Address;
      formatShortAddress(addr);
    }
