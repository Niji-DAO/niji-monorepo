...
describe('resolveNijiContractAddress', () => {
  ...
  it('round-6 30 sequential DAO resolver calls', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 30; i++) {
  ...
  it('round-6 50 sequential AuctionHouse resolver calls', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 50; i++) {
  ...
  it('round-6 100 sequential resolver type checks', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 100; i++) {
  ...
  it('round-6 30 deterministic for same contract name', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 30; i++) {
  ...
  it('round-6 100 sequential alternating call cycles', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 100; i++) {
      const name = i % 2 === 0 ? 'DAO' : 'AuctionHouse';
      expect(() => resolveNijiContractAddress(name as never)).not.toThrow();
  ...
  it('round-8 30 sequential resolveNijiContractAddress calls', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 30; i++) {
      expect(() => resolveNijiContractAddress('DAO' as never)).not.toThrow();
    }
  ...
  it('round-8 100 sequential alternating call cycles', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 100; i++) {
      const name = i % 2 === 0 ? 'DAO' : 'AuctionHouse';
      expect(() => resolveNijiContractAddress(name as never)).not.toThrow();
  ...
  it('round-11 30 sequential resolveNijiContractAddress truthiness', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 30; i++) {
      expect(resolveNijiContractAddress).toBeTruthy();
    }
  ...
  it('round-11 50 sequential invocations', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 50; i++) {
      expect(() => resolveNijiContractAddress(TRE)).not.toThrow();
    }
  ...
  it('round-11 100 mixed name variation', async () => {
    const { resolveNijiContractAddress } = await importResolver();
    for (let i = 0; i < 100; i++) {
      const addr = i % 3 === 0 ? GOV : i % 3 === 1 ? AH : TRE;
      expect(() => resolveNijiContractAddress(addr)).not.toThrow();
