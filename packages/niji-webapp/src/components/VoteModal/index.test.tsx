...
describe('VoteModal', () => {
  ...
  it('round-10 30 sequential VoteModal mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<VoteModal {...baseProps} />);
      unmount();
    }
  ...
  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<VoteModal {...baseProps} availableVotes={i + 30000} />);
      unmount();
    }
  });
  ...
  it('round-12 30 sequential VoteModal mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<VoteModal {...baseProps} />);
      unmount();
    }
