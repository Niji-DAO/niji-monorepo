...
describe('NijiInfoCard', () => {
  ...
  it('round-10 30 sequential NijiInfoCard mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<NijiInfoCard nounId={1n} bidHistoryOnClickHandler={() => {}} />);
      unmount();
    }
  ...
  it('round-10 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <NijiInfoCard key={i} nounId={BigInt(i + 1)} bidHistoryOnClickHandler={() => {}} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<NijiInfoCard nounId={2n} bidHistoryOnClickHandler={() => {}} />);
      unmount();
    }
  });
  ...
  it('round-10 100 sequential handler invocations', () => {
    const cb = vi.fn();
    render(<NijiInfoCard nounId={3n} bidHistoryOnClickHandler={cb} />);
    for (let i = 0; i < 100; i++) cb();
    expect(cb).toHaveBeenCalledTimes(100);
  });

  it('round-11 30 sequential NijiInfoCard mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <NijiInfoCard nounId={BigInt(i)} bidHistoryOnClickHandler={() => {}} />,
      );
  ...
  it('round-11 30 different nounId values', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <NijiInfoCard nounId={BigInt(i + 5000)} bidHistoryOnClickHandler={() => {}} />,
      );
      unmount();
    }
  });
  ...
  it('round-11 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<NijiInfoCard nounId={4n} bidHistoryOnClickHandler={() => {}} />);
      unmount();
    }
  });
