...
describe('BidHistory Component', () => {
  ...
  it('round-8 renders 30 instances variant', () => {
    ...
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <BidHistory key={i} auctionId={`${i + 15000}`} max={3} classes={mockClasses} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-8 30 different auctionId values', () => {
    vi.mocked(useAuctionBids).mockReturnValue(undefined);
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <BidHistory auctionId={`${i + 17000}`} max={3} classes={mockClasses} />,
      );
      unmount();
    }
  });
  ...
  it('round-9 renders 30 instances variant', () => {
    ...
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <BidHistory key={i} auctionId={`${i + 19000}`} max={3} classes={mockClasses} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-9 30 different auctionId values', () => {
    vi.mocked(useAuctionBids).mockReturnValue(undefined);
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <BidHistory auctionId={`${i + 21000}`} max={3} classes={mockClasses} />,
      );
      unmount();
    }
  });

  it('round-10 30 sequential BidHistory mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
  ...
  it('round-10 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <BidHistory key={i} auctionId={`${i + 51000}`} max={3} classes={mockClasses} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-10 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() =>
        render(<BidHistory auctionId={`${i + 52000}`} max={3} classes={mockClasses} />),
      ).not.toThrow();
    }
  });
  ...
  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(
        <BidHistory auctionId={`${i + 53000}`} max={3} classes={mockClasses} />,
      );
      unmount();
    }
  });

  it('round-10 100 sequential different auctionId values', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(
        <BidHistory auctionId={`${i + 40000}`} max={3} classes={mockClasses} />,
      );
      unmount();
    }
  });
