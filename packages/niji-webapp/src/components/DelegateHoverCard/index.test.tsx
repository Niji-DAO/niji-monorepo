...
describe('DelegateHoverCard', () => {
  ...
  it('round-9 mount-unmount 30 cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<DelegateHoverCard delegateId="0xR9" proposerAddress="0xPROP" />);
      unmount();
    }
  });
  ...
  it('round-9 renders 30 instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <DelegateHoverCard key={i} delegateId={`0xR9-${i}`} proposerAddress="0xPROP" />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-9 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() =>
        render(<DelegateHoverCard delegateId="0xR9" proposerAddress="0xPROP" />),
      ).not.toThrow();
    }
  });

  it('round-9 30 different delegateId values second', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <DelegateHoverCard delegateId={`0xR9-d-${i}`} proposerAddress="0xPROP" />,
      );
      unmount();
    }
  });
  ...
  it('round-9 50 mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(
        <DelegateHoverCard delegateId="0xR9-2" proposerAddress="0xPROP" />,
      );
      unmount();
    }
  });
