...
describe('BidHistoryItem Component', () => {
  ...
  it('round-2 renders 200 instances in ul', () => {
    expect(() =>
      render(
        <ul>
          {Array.from({ length: 200 }, (_, i) => (
  ...
  it('round-2 all 100 instances render link-icon', () => {
    const { container } = render(
      <ul>
        {Array.from({ length: 100 }, (_, i) => (
          <BidHistoryItem key={i} bid={mockBid} classes={mockClasses} />
  ...
  it('round-2 handles 30 different bid timestamps', () => {
    for (let i = 0; i < 30; i++) {
      const b = { ...mockBid, timestamp: BigInt(1700000000 + i * 86400) };
      const { unmount } = render(<BidHistoryItem bid={b} classes={mockClasses} />);
      unmount();
  ...
  it('round-2 handles 30 different isCool combinations', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <BidHistoryItem bid={mockBid} classes={mockClasses} isCool={i % 2 === 0} />,
      );
  ...
  it('round-10 30 sequential BidHistoryItem mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<BidHistoryItem bid={mockBid} classes={mockClasses} />);
      unmount();
    }
  ...
  it('round-12 30 sequential BidHistoryItem mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<BidHistoryItem bid={mockBid} classes={mockClasses} />);
      unmount();
    }
