...
describe('LegacyNoun — additional edge cases', () => {
  ...
  it('round-10 30 sequential LegacyNoun mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<LegacyNoun imgPath="/r10.png" alt="r10" />);
      unmount();
    }
  });
  ...
  it('round-10 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <LegacyNoun key={i} imgPath="/r10.png" alt={`r10-${i}`} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-10 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() => render(<LegacyNoun imgPath="/r10.png" alt="r10" />)).not.toThrow();
    }
  });

  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<LegacyNoun imgPath="/r10-2.png" alt="r10-2" />);
      unmount();
    }
  });

  it('round-10 100 sequential different alt values', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(<LegacyNoun imgPath="/r10.png" alt={`r10-c-${i}`} />);
      unmount();
    }
  });
});
