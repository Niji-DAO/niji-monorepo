...
describe('MinBid', () => {
  ...
  it('round-5 50 different minBid values', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<MinBid minBid={BigInt(i + 5000)} onClick={() => {}} />);
      unmount();
    }
  ...
  it('round-6 rapid 200 onClick invocations', () => {
    const onClick = vi.fn();
    render(<MinBid minBid={1n} onClick={onClick} />);
    for (let i = 0; i < 200; i++) onClick();
    expect(onClick).toHaveBeenCalledTimes(200);
  ...
  it('round-8 renders 30 instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <MinBid key={i} minBid={BigInt(i + 10000)} onClick={() => {}} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-9 renders 30 instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <MinBid key={i} minBid={BigInt(i + 12000)} onClick={() => {}} />
          ))}
        </>,
      ),
    ).not.toThrow();
