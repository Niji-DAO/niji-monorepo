...
describe('CurrentBid', () => {
  ...
  it('round-5 100 rerender cycles', () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(() =>
        rerender(<CurrentBid currentBid={BigInt(i + 7000) * 10n ** 18n} auctionEnded={false} />),
      ).not.toThrow();
    }
  });
  ...
  it('round-6 100 rerender cycles', () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(() =>
        rerender(<CurrentBid currentBid={BigInt(i + 9000) * 10n ** 18n} auctionEnded={false} />),
      ).not.toThrow();
    }
  });
  ...
  it('round-7 renders 30 instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <CurrentBid key={i} currentBid={BigInt(i + 11000) * 10n ** 18n} auctionEnded={false} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-7 100 rerender cycles', () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(() =>
        rerender(<CurrentBid currentBid={BigInt(i + 13000) * 10n ** 18n} auctionEnded={false} />),
      ).not.toThrow();
    }
  });
  ...
  it('round-8 renders 30 instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <CurrentBid key={i} currentBid={BigInt(i + 15000) * 10n ** 18n} auctionEnded={false} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-8 100 rerender cycles', () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(() =>
        rerender(<CurrentBid currentBid={BigInt(i + 17000) * 10n ** 18n} auctionEnded={false} />),
      ).not.toThrow();
    }
  });
  ...
  it('round-10 30 sequential CurrentBid mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<CurrentBid currentBid={1n * 10n ** 18n} auctionEnded={false} />);
      unmount();
    }
  ...
  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(
        <CurrentBid currentBid={BigInt(i + 30000) * 10n ** 18n} auctionEnded={false} />,
      );
      unmount();
    }
  });
  ...
  it('round-10 100 sequential mount-unmount cycles', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(
        <CurrentBid currentBid={BigInt(i + 40000) * 10n ** 18n} auctionEnded={true} />,
      );
      unmount();
    }
  });
