...
describe('BidHistoryModalRow', () => {
  ...
  it('round-12 30 sequential BidHistoryModalRow mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<BidHistoryModalRow bid={bid} index={i + 100000} />);
      unmount();
    }
  });
  ...
  it('round-12 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <BidHistoryModalRow key={i} bid={bid} index={i + 110000} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-12 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() => render(<BidHistoryModalRow bid={bid} index={i + 120000} />)).not.toThrow();
    }
  });

  it('round-12 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<BidHistoryModalRow bid={bid} index={i + 130000} />);
      unmount();
    }
  });
  ...
  it('round-12 100 sequential different index values', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(<BidHistoryModalRow bid={bid} index={i + 140000} />);
      unmount();
    }
  });
});
