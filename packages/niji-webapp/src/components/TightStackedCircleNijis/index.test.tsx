...
describe('TightStackedCircleNijis', () => {
  ...
  it('round-10 30 sequential TightStackedCircleNijis mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<TightStackedCircleNijis nounIds={[String(i)]} />);
      unmount();
    }
  });
  ...
  it('round-10 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <TightStackedCircleNijis key={i} nounIds={[String(i)]} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-10 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() => render(<TightStackedCircleNijis nounIds={[String(i)]} />)).not.toThrow();
    }
  });

  it('round-10 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<TightStackedCircleNijis nounIds={[String(i + 30000)]} />);
      unmount();
    }
  });

  it('round-10 100 sequential different nounIds values', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(<TightStackedCircleNijis nounIds={[String(i + 40000)]} />);
      unmount();
    }
  });
