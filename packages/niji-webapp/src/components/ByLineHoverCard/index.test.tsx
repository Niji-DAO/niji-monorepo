...
describe('ByLineHoverCard', () => {
  ...
  it('round-12 30 sequential ByLineHoverCard mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<ByLineHoverCard proposerAddress={`0xR12-m-${i}`} />);
      unmount();
    }
  });
  ...
  it('round-12 30 renders instances variant', () => {
    expect(() =>
      render(
        <>
          {Array.from({ length: 30 }, (_, i) => (
            <ByLineHoverCard key={i} proposerAddress={`0xR12-i-${i}`} />
          ))}
        </>,
      ),
    ).not.toThrow();
  ...
  it('round-12 30 sequential renders without crash', () => {
    for (let i = 0; i < 30; i++) {
      expect(() => render(<ByLineHoverCard proposerAddress={`0xR12-s-${i}`} />)).not.toThrow();
    }
  });

  it('round-12 50 sequential mount-unmount cycles second', () => {
    for (let i = 0; i < 50; i++) {
      const { unmount } = render(<ByLineHoverCard proposerAddress={`0xR12-m2-${i}`} />);
      unmount();
    }
  });

  it('round-12 100 sequential different proposerAddress values', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(<ByLineHoverCard proposerAddress={`0xR12-c-${i}`} />);
      unmount();
    }
  });
});
