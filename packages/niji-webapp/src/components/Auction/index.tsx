...
import { stateBackgroundColorAtom } from '@/state/atoms/applicationAtom';
...
const Auction: React.FC<AuctionProps> = props => {
  ...
  const setStateBackgroundColor = useSetAtom(stateBackgroundColorAtom);
  const lastNounId = useAtomValue(lastAuctionNounIdAtom);

  const loadedNounHandler = (seed: INounSeed) => {
    setStateBackgroundColor(seed.background === 0 ? grey : beige);
  };
