...
describe('SignatureFormFields', () => {
  ...
  it('round-5 rapid 300 setReasonText invocations', () => {
    const setReasonText = vi.fn();
    const { container } = render(
      <SignatureFormFields {...defaults} setReasonText={setReasonText} />,
    );
    const textarea = container.querySelector('textarea')!;
    for (let i = 0; i < 300; i++) {
      fireEvent.change(textarea, { target: { value: `r5-r-${i}` } });
    }
    expect(setReasonText).toHaveBeenCalledTimes(300);
  });
  ...
  it('round-9 100 sequential mount cycles second', () => {
    for (let i = 0; i < 100; i++) {
      const { unmount } = render(<SignatureFormFields {...defaults} />);
      unmount();
    }
  ...
  it('round-10 30 sequential SignatureFormFields mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<SignatureFormFields {...defaults} />);
      unmount();
    }
  ...
  it('round-12 30 sequential SignatureFormFields mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(<SignatureFormFields {...defaults} />);
      unmount();
    }
