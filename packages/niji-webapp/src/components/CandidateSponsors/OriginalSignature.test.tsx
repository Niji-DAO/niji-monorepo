...
describe('OriginalSignature', () => {
  ...
  it('round-4 30 different voteCount values', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={i + 500} signer={SIGNER} isParentProposalUpdatable={true} />,
      );
      unmount();
    }
  });

  it('round-4 30 isParentProposalUpdatable toggle cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={1} signer={SIGNER} isParentProposalUpdatable={i % 2 === 0} />,
      );
  ...
  it('round-5 30 isParentProposalUpdatable toggle cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={1} signer={SIGNER} isParentProposalUpdatable={i % 2 === 0} />,
      );
  ...
  it('round-6 30 different voteCount values', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={i + 9000} signer={SIGNER} isParentProposalUpdatable={true} />,
      );
      unmount();
    }
  });

  it('round-6 30 isParentProposalUpdatable toggle cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={1} signer={SIGNER} isParentProposalUpdatable={i % 2 === 0} />,
      );
  ...
  it('round-10 30 sequential OriginalSignature mount-unmount cycles', () => {
    for (let i = 0; i < 30; i++) {
      const { unmount } = render(
        <OriginalSignature voteCount={1} signer={SIGNER} isParentProposalUpdatable={true} />,
      );
